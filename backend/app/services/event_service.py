...
class EventService:
    ...
    def __init__(
        ...
        visitor_history_svc: Optional[VisitorHistoryService] = None,
    ):
        self._registry = registry
        self._event_repo = event_repo
        self._alert_repo = alert_repo
        self._visitor_history_svc = visitor_history_svc
    ...
    async def ingest_event(
        ...
        # Sprint 13: Record visitor movement event and update visits/profiles if visitor identity is present
        visitor_id = getattr(request, "visitor_id", None) or getattr(request, "identity_id", None)
        if visitor_id and visitor_id != "UNKNOWN" and self._visitor_history_svc:
            try:
                await self._visitor_history_svc.record_movement_event(
                    venue_id=venue_id,
                    session_id=session_id,
                ...
                logger.error(f"Failed to record visitor history for visitor '{visitor_id}': {vhe}")
