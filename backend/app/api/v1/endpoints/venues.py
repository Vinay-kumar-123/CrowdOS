"""
Venue Endpoints — Sprint 10.

REST API for venue management with MongoDB persistence.

Routes:
    GET  /v1/venues                   → list registered venues
    GET  /v1/venues/{venue_id}        → venue info + engine status
    POST /v1/venues/{venue_id}/reset  → reset venue engine state (testing utility)
"""
...
from app.dependencies.database import get_venue_repository
...
def _get_venue_service(
    ...
    return VenueService(venue_registry, venue_repo=venue_repo)
...
async def get_venue_info(
    ...
    registered_list = await svc.list_venues()
    ...
    return VenueInfoResponse(
        ...
        registered_venues=len(registered_list),
    )
